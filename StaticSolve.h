    ...
    double &reg)
{
    ...
    while (true)
    {
        ...
        {
            Eigen::VectorXd derivativeEnzyme;
            std::vector<Eigen::Triplet<double> > hessianEnzyme;
            double energyEnzyme = elasticEnergyEnzyme(mesh, curPos, curEdgeDOFs, mat, restState, &derivativeEnzyme, &hessianEnzyme);
            std::cout << "Derivative difference: " << (derivativeEnzyme - derivative).norm() << std::endl;
        }
